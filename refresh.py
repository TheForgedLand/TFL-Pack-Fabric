...
def update_flavors(modlist):
    ...
    data = toml.load(flavor_groups) | {"metafile": {
        k: {"flavors": v} for k, v in fl.items()}}  # type: ignore

    newly_added = {(mm if mm not in fl.keys() else ...)for mm in modlist}  # type: ignore
    newly_added.remove(...)  # type: ignore
    outdated = {(mod if mod not in modlist else ...)for mod in modflavors.keys()}
    outdated.remove(...)
    ...
    for mod in outdated:
        for m in modlist:
            if le.ratio(mod, m) >= 0.85: # type: ignore
                ...
                print('%s: %s' % (mod, modflavors.get(mod)))  # type: ignore
        modflavors.pop(mod)
...
custom_sources = [
    ...
    ["origins-plus-plus",
        "https://github.com/AliCCfire/TFL-modpack/raw/CMOD/origins-plus-plus-2.0.1.jar"],
